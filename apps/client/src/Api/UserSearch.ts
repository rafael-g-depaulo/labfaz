...
const sortUsers = (userA: User, userB: User): number => {
  ...
  if ((userA as any).isVerified && !(userB as any).isVerified) return -1
  if ((userB as any).isVerified && !(userA as any).isVerified) return 1
...
const isLGBTQ = (user: User) => 
  !(user.artist.gender_specific! === GenderSpecific.CIS || user.artist.gender_specific! === GenderSpecific.NONE) ||
...
const isInArea = (user: User, area: string) => compareStrings(user.artist?.technical?.areas?.[0]?.name!, area)
...
const hasCurriculum = (user: User) => !!(user.artist as any).curriculum && (user.artist as any).curriculum !== ""
